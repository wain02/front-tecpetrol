...
export function PreAperturaFitChart({
  ...
  const linealCurve =
    displayEvent.a_lineal != null && displayEvent.b_lineal != null
      ? tFine.map((t) => displayEvent.a_lineal! * t + displayEvent.b_lineal!)
      : null;
  ...
  return (
    <div className="chart">
      ...
      <svg
        ...
        role="img"
        aria-label={`Ajuste de pre-apertura — ${event.pozo_id}`}
      >
        {yTicks.map((tick) => {
          const y = ys(tick);
          return (
        ...
        {displayEvent.P_estimada_lineal != null ? (
          <text x={x0} y={ys(displayEvent.P_estimada_lineal)} textAnchor="middle" dominantBaseline="middle" fill="#16a34a" fontSize="20" fontWeight="bold">
            ★
          </text>
        ) : null}

        <text x={pad.left + cw / 2} y={height - 8} textAnchor="middle" className="chart__axis-label" fontWeight="600">
          Horas antes de la apertura (izq = más antiguo, der = apertura)
        </text>
      ...
      <div className="pba-fit-footer">
        <div className="chart__legend">
          ...
          {linealCurve ? (
            <div className="chart__legend-item">
              <span style={{ width: 18, display: "inline-block", borderTop: "2.2px dashed #16a34a", marginTop: 6 }} />
              <span>Lineal</span>
            </div>
          ) : null}
          {expCurve ? (
        ...
        <div className="pba-fit-metrics">
          {displayEvent.P_estimada_lineal != null ? (
            <div className="pba-fit-metric">
              <span className="pba-fit-metric__star" style={{ color: "#16a34a" }}>★</span>
              <div>
                <strong>P(t=0) Lineal = {displayEvent.P_estimada_lineal.toFixed(1)} psi</strong>
                <span>R² = {displayEvent.R2_lineal?.toFixed(3) ?? "—"}</span>
              </div>
            </div>
          ) : null}
          {displayEvent.P_estimada_exp != null ? (
